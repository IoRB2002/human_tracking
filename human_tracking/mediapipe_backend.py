...
class MediaPipeTrackingBackend:
    def __init__(
        ...
        *,
        ...
        self.pose_model_path = Path(pose_model_path).expanduser().resolve()
        self.hand_model_path = Path(hand_model_path).expanduser().resolve()
        ...
        for name, value in (
            ...
            ("min_hand_tracking_confidence", min_hand_tracking_confidence)
        ):
        ...
        self.min_hand_tracking_confidence = float(min_hand_tracking_confidence)
    ...
    def process(self, frame: CameraFrame) -> MediaPipeTrackingOutput:
        if not np.isfinite(frame.measurement_timestamp_s):
            raise ValueError("Camera frame timestamp must be finite.")

        if frame.measurement_timestamp_s < 0.0:
            raise ValueError("Camera frame timestamp must be non-negative.")

        if self._last_sequence_id is not None:
            if frame.sequence_id <= self._last_sequence_id:
                raise ValueError("Camera frame sequence IDs must increase.")

        # MediaPipe VIDEO mode uses integer milliseconds.
        timestamp_ms = int(frame.measurement_timestamp_s * 1000.0)
        ...
        processing_duration_s = (time.perf_counter() - processing_start_s)
    ...
    def __enter__(self) -> "MediaPipeTrackingBackend":
        return self
